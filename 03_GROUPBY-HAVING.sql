 ...
 * 1 : FROM 참조할 테이블명
 * 2 : WHERE 절 컬럼명 | 함수식 비교연산자 비교값
 * 3 : GROUP BY 그룹을 묶을 컬럼명
 * 4 : HAVING 그룹함수식 비교연산자 비교값
 * 6 : ORDER BY 컬럼명 | 별칭 | 컬럼 순번 정렬방식[NULLS FIRST | LAST];
 * 
 * */
...
-- 그룹으로 묶은 값에 대해서 SECECT 절 그룹함수를 사용함
...
-- GROUP BY는 별칭 사용 X (SELECT절 해석 X)
-- ORDER BY는 별칭 사용 O (SELECT절 해석 완료)
...
-- EMPLOYEE 테이블에서 직급별 2000년도 이후 입사자들의 급여합을 조회
--(직급코드로 오름차순)
...
-- * HAVING 절 : 그룹 함수로 구해 올 글부에 대한 조건을 설정할 때 사용
...
/* SET OPERATOR(집합 연산자)
 * 
 * -- 여러 SELECT의 결과(RESULT SET)를 하나의 연산자로 만드는 연산자
 * 
